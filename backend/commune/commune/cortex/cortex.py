...
from commune.cortex.utils import load_yaml, save_yaml, load_json, save_json
import threading
import time
import queue
...
class Cortex:
    def __init__(self, 
        ...
        self.config.bittensor =  bittensor.config(parser = self.argparser())
    ...
    def set_receptor_pool(self, receptor_pool=None, refresh=None):
    ...
    def set_tokenizer(self, tokenizer:Optional[bittensor.tokenizer]=None):
        if tokenizer == None:
    ...
    async def async_sample(self,
        ...
        raw_inputs = self.dataset.sample( batch_size=batch_size, idx_list = idx_list, sequence_length=sequence_length, tokenize= False)['text']
...
if __name__ == '__main__':
    module = Cortex()
    st.write(module.__file__)

    # module.sample(batch_size=10, num_endpoints=100, timeout=2,sequence_length=10)
