...
export default class Navbar extends Component{
    ...
    onDragStart = (event, nodeType, item={}) => {
        ...
        if(!["customInput", "space"].includes(nodeType)){
    ...
    subComponents(item, index){
        return(<>
                <li key={`${index}-li`} onDragStart={(event) => this.state.disable ? this.onDragStart(event, 'process', item) : this.onDragStart(event, 'custom', item)} 
                    className={` text-white text-md flex flex-col text-center items-center cursor-grab  shadow-sm hover:shadow-xl
        ...
        return (<>
                <li onDragStart={(event) => param.onDragStart(event, 'customInput', {})}
                    className={`text-md flex flex-col text-center items-center cursor-grab shadow-sm hover:shadow-xl
                                 p-2 px-2 mt-4 mb-1 rounded-md ${ param.open ? `border-black dark:border-blue-50 border-2 dark:bg-gradient-to-tr from-stone-900 to-stone-700/5` : `hidden`}  break-all -z-20 duration-300`} draggable={true}>
                <div className="w-full hover:animate-pulse bg-transparent ">
        ...
        return (<div>
            ...
            <BsArrowLeftShort onClick={this.handelNavbar} className={` fixed bg-white text-Retro-darl-blue text-3xl rounded-full top-9 border border-black cursor-pointer ${this.state.open ? 'rotate-180 left-[22.5rem]' : 'left-7' } dark:border-white duration-300 dark:text-white dark:bg-stone-900 z-[1000] `}/>
                ...
                <div className={`${this.state.open ? 'mb-5' : 'hidden'} flex`}>
                    <div className={` w-14 h-7 flex items-center border-2 bg-slate-700  hover:shadow-xl rounded-full p-1 cursor-pointer float-left duration-300 `} onClick={() => {this.handelDisable()}}>
                        <div className={`h-[1.5rem] w-[1.5rem] rounded-full shadow-md transform duration-300 ease-in-out hover:ring-2 hover:ring-[#965cf9] hover:ring-opacity-30 hover:shadow-xl   bg-white  ${this.state.disable ? ' transform -translate-x-[0.18rem]' : "transform translate-x-[1.36rem] "}`}></div>
                    </div>

                    <div className={` ml-2 w-14 h-7 flex items-center ${this.state.disable && "opacity-50" } border-2 ${this.state.toggle === "gradio" ? 'bg-white border-orange-400' : ' bg-slate-800'} hover:shadow-xl rounded-full p-1 cursor-pointer float-left duration-300 `} onClick={() => {if (!this.state.disable) this.handelToggle()}}>
                        <Streamlit className=" absolute w-5 h-5"/>
                        <Gradio className=" absolute w-5 h-5 translate-x-6 shadow-sm hover:shadow-2xl"/>
                ...
                <div className={`flex w-full border-2 border-solid border-black rounded-lg dark:border-white  duration-300 hover:shadow-2xl ${this.state.open ? 'mb-5' : 'hidden'}`} draggable onDragStart={(event) => this.onDragStart(event, 'space')}>
                    <div className="flex w-full dark:bg-gradient-to-l from-stone-900 to-stone-800/5 hover:animate-pulse rounded-lg">
                ...
                <div className={` h-[80%] ${this.state.loading ? "overflow-hidden" : "overflow-auto"}`}>
                    <div id="module-list" className={` relative z-10 w-full ${this.state.loading ? " animate-pulse duration-300 dark:bg-stone-900 bg-slate-100 rounded-lg bottom-0" : ""} `}>
