        ...
        return (<div>
            ...
            <BsArrowLeftShort onClick={this.handelNavbar} className={` fixed bg-white text-Retro-darl-blue text-3xl rounded-full top-9 border border-black cursor-pointer ${this.state.open ? 'rotate-180 left-[22.5rem]' : 'left-7' } dark:border-white duration-300 dark:text-white dark:bg-stone-900 z-[1000] `}/>
                ...
                <div className={`${this.state.open ? 'mb-5' : 'hidden'} flex`}>
                    <div className={` w-14 h-7 flex items-center border-2 bg-slate-700  shadow-xl rounded-full p-1 cursor-pointer float-left duration-300 `} onClick={() => {this.handelDisable()}}>
                        <div className={`h-[1.5rem] w-[1.5rem] rounded-full shadow-md transform duration-300 ease-in-out hover:ring-4 hover:ring-[#965cf9] hover:ring-opacity-60 hover:shadow-lg   bg-white  ${this.state.disable ? ' transform -translate-x-[0.18rem]' : "transform translate-x-[1.36rem] "}`}></div>
                    </div>
